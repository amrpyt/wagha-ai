...
import type { InteriorModifiers, InteriorTemplate } from '@/lib/ai/prompts'
...
const INTERIOR_TEMPLATES: { value: InteriorTemplate; label: string }[] = [
  ...
  { value: 'retail', label: 'متجري' },
]

export default function InteriorPage() {
  // ── Fixed interior render type (D-07) ─────────────────────────────
  const renderType: 'interior' = 'interior'

  // ── Template state (dropdown selection) ─────────────────────────
  const [template, setTemplate] = useState<InteriorTemplate>('residential')
  const [modifiers, setModifiers] = useState<InteriorModifiers>(() => getTemplateModifiers('interior', 'residential') as InteriorModifiers)
  ...
  const handleModifiersChange = (m: InteriorModifiers) => {
    setModifiers(m)
  }
  ...
  return (
    <div className="flex gap-6 min-h-[calc(100vh-120px)]">
      {/* ── Left Panel (interior options only, D-07) ────────────── */}
      <InteriorLeftPanel
        modifiers={modifiers}
      ...
      <div className="flex-1 max-w-4xl space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>/</span>
          <span className="text-gray-900">تصميم داخلي</span>
        </div>

        {/* Template selector (D-16 replacement for TemplatePills) */}
